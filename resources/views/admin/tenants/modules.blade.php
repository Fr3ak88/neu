...
@section('title', 'Module — ' . $tenant->company)

@section('content')
<div class="page-header">
    <div class="page-title">Module für {{ $tenant->company }}</div>
    <div class="page-subtitle">Aktivierte Module verwalten</div>
</div>

<div style="max-width:32rem">
    <div class="card">
        <form method="POST" action="{{ route('admin.tenants.update-modules', $tenant) }}">
            @csrf
            @method('PUT')
            ...
            @foreach($available as $key => $label)
                ...
                <label style="display:flex;align-items:center;gap:var(--space-3);padding:var(--space-3) var(--space-4);border:1px solid var(--color-border);border-radius:var(--radius-md);margin-bottom:var(--space-3);cursor:pointer;transition:all var(--transition)">
                    <input type="checkbox" name="modules[]" value="{{ $key }}" {{ in_array($key, $tenant->modules ?? []) ? 'checked' : '' }} style="width:16px;height:16px">
                    <div>
